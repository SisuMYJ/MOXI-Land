...
export class IslandScene extends Phaser.Scene {
  ...
  create() {
    ...
    [80, 210, 450].forEach((x, i) => {
      const cloud = this.add.text(x, 70 + i * 26, '☁️', { fontSize: '38px' }).setAlpha(0.75);
      this.tweens.add({ targets: cloud, x: x + 80, duration: 12000 + i * 3500, yoyo: true, repeat: -1 });
    });

    for (let i = 0; i < 11; i += 1) {
      const tree = this.add.text(70 + (i % 4) * 125, 150 + Math.floor(i / 4) * 120, '🌳', { fontSize: '34px' });
      this.tweens.add({ targets: tree, angle: 4, duration: 1100 + i * 90, yoyo: true, repeat: -1 });
    }

    this.add
      .text(w / 2, 118, '森林探险区', { fontSize: '18px', color: '#315342' })
      .setOrigin(0.5)
    ...
    this.add
      .text(w / 2, 690, '星光湖 ✨', { fontSize: '18px', color: '#26566b' })
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true })
    ...
    [
      ...
      const hex = this.add
        .polygon(x, y, [35, 0, 17, 30, -17, 30, -35, 0, -17, -30, 17, -30], 0xffffff, 0.25)
    ...
    pickDailyItems(residents, { count: residentCount, namespace: 'outside-residents', date: today }).forEach((resident) => {
      new ResidentSprite(this, resident, () => emitIslandEvent('moxi-open-resident', resident.id));
    });
  }
}
