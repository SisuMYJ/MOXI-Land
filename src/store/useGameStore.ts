...
import { boardMessages } from '../content/messages';
...
export const useGameStore = create<State>((set,get)=>({ ...persisted,
 ...
 viewBoard:()=>set(s=>{ const today=todayKey(); if(s.board?.date===today) return s; if(s.moons<1){get().notify('查看留言板需要 1 月亮币'); return s;} const board={date:today,message:`${s.weather}：${boardMessages[daySeed()%boardMessages.length]}`,viewed:true}; const ns={...s,moons:s.moons-1,board} as State; persist(ns); return ns;}),
